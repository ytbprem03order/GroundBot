...
async def get_charge_resp(result, user_id, fullcc):
    try:
        ...
        if type(result) == str:
            ...
            or "Thank You for your order," in result.text
            or "Thank You For Your Order," in result.text
            or "order-received" in result.text
        ):
            status   = "𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅"
            response = "Charged 5$ 🔥"
            hits     = "YES"
            await forward_resp(fullcc, "SITEBASE CVV Charge 5$", response)
            ...
            status   = "𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌"
            response = "The card was declined."
            hits     = "NO"
        ...
        elif "call_issuer" in result.text:
            status   = "𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌"
            response = "Call issuer "
            hits     = "NO"
