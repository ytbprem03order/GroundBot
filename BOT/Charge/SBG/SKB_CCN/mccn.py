...
async def mccnfunc(fullcc , sks , user_id , session):
    try:  
...
- 𝐒𝐭𝐚𝐭𝐮𝐬 - Processing...⌛️
        ...
        text = f"""
<b> MASS SKBASE 1$ CVV CHARGE [/mccn]
...
Number Of CC Check : [{len(ccs)} / 25]
        ...
        sks        = await getallsk()
        works      = [mccnfunc(i, sks , user_id , session) for i in ccs]
