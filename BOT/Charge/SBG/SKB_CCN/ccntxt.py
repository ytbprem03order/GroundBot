...
@Client.on_message(filters.command("ccntxt", [".", "/"]))
...
async def stripe_mass_txt_auth_cmd(Client, message):
    ...
    try:
        ...
        try:
            ...
            key = f"ccntxt{message.from_user.id}_{random_text}"
            file_name = f"{key}.txt"
            hitsfile = f"HITS/{file_name}"
            await message.reply_to_message.download(file_name=file_name)
        except:
            resp = """<b>
Gate Name: SK BASED 1$ CCN ♻️
CMD: /ccntxt

Message: No CC Found in your input ❌

Usage: /ccntxt [ in reply to txt file ]
        </b> """
